...
class VideoAssembler:
    ...
    def _extract_scenes(
        ...
        for i, scene in enumerate(scenes):
            ...
            # FIXED: Normalize video to 1280x720 canvas first to guarantee watermark coords always fit
            vf_parts.append("scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:(ow-iw)/2:(oh-ih)/2")

            # Add watermark removal (now safely bounded inside the 1280x720 canvas)
            if watermark_regions:
                for region in watermark_regions:
                    x, y, w, h = region["x"], region["y"], region["w"], region["h"]
                    vf_parts.append(f"delogo=x={x}:y={y}:w={w}:h={h}")

            # Add scaling/cropping for final social media format
            if fmt.crop_filter:
